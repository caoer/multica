    ...
    entries: [
      {
        version: "0.3.11",
        date: "2026-05-28",
        title: "计费、MCP 配置与更安全的本地运行",
        changes: [],
        features: [
          "计费功能补齐了余额、交易、充值、结账和 Stripe 回调的服务端通道，并提供验证页面帮助确认流程",
          "智能体详情页新增 MCP 配置入口，Hermes、Kimi 和 Kiro 运行环境会自动使用这些配置",
          "Multica 现在支持 Antigravity 作为新的编码运行环境，使用 Google CLI 的团队可以直接接入",
          "自托管管理员可以关闭自助创建工作区，用户在创建入口会看到清晰的联系管理员提示",
          "桌面端本机运行服务会在访问令牌过期前自动续期，长期在线的本机运行环境不再容易突然失联",
        ],
        improvements: [
          "自托管团队可以从 GHCR 发布和安装 Helm Chart，Microsoft Exchange 与 SMTP Relay 邮件配置文档也更完整",
          "评论触发的智能体运行会提示上次之后的新评论，并默认接上前一次对话上下文",
          "任务记录里的工作目录现在用更安全的相对路径显示，长路径可复制，分段消息也会合并成更顺畅的时间线",
          "Issue 看板在手动排序时会把新 Issue 放到列顶部，最近删除的 Issue 也不会再回到列表中",
          "本机运行环境列表会按设备名合并同一台机器，减少重复项",
        ],
        fixes: [
          "任务完成或失败回传遇到短暂错误时会自动重试，减少一直停在运行中的情况",
          "绑定本地目录运行时，Multica 不会再覆盖已有的 CLAUDE.md、AGENTS.md 或 GEMINI.md 指引文件",
          "Windows 上的 Pi 运行可以正确保留多行提示词",
          "不同构建环境里的运行环境 Logo 显示更稳定",
          "本机运行服务清理任务时会安全跳过不完整的父级信息",
        ],
      },
      {
        version: "0.3.10",
